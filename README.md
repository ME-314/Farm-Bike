...
The bottom bracket shell attaches the standard BSA bicycle bottom bracket to the frame. The part is a weldment of a round bottom bracket shell recovered from a donor frame and a 40 mm $\times$ 30 mm rectangular steel tube. These dimensions provide a good compromise between stiffness, compactness and ease of integration with the aluminium profile frame. The 40 mm side is consistent with the 40 mm modular aluminium profiles used for the main structure, which simplifies alignment and mounting. The 30 mm height keeps the bottom bracket support compact while still providing enough section height to resist the moments generated by pedalling forces. A smaller tube would have reduced stiffness and weld area around the bottom bracket shell, while a larger section would have increased weight and made the assembly bulkier without a clear functional benefit. The part requires far less material resources than an equivalent machined from a billet and does not require specialty tooling to cut the BSA thread.
#### Hub
